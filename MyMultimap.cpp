...
class MyMultimap {
private:
    RedBlackTree<KEY, std::vector<VALUE>> rbtree;
    size_t totalsize;
public:
    MyMultimap() :rbtree(), totalsize(0) {

    }

    void insert(const KEY& key, const VALUE& value) {
        auto foundvalues = rbtree.at(key);
        if (foundvalues != nullptr) {
            (*foundvalues).push_back(value);
        }
        else {
            rbtree.insert(key, std::vector<VALUE>{value});
        }
        ++totalsize;
    }
    ...
    void remove(const KEY& key, const VALUE& value) {
        auto foundvalues = rbtree.at(key);
        if (foundvalues != nullptr) {
            auto& values = *foundvalues;
            size_t before = values.size();
            values.erase(std::remove(values.begin(), values.end(), value), values.end());
            size_t now = values.size();
            totalsize -= (before - now);
            if (values.empty()) {
                rbtree.remove(key);
            }
        }
    ...
    void remove(const KEY& key) {
        auto foundvalues = rbtree.at(key);
        if (foundvalues != nullptr) {
            size_t num = (*foundvalues).size();
            rbtree.remove(key);
            totalsize -= num;
        }
    }

    std::vector<VALUE>* at(const KEY& key) {
...
int main(int argc, char* argv[])
{
    ...
    while (N--) {
        ...
        if (command == "insert") {
            ...
            iss >> key;
            auto foundvalues = gaogao.at(key);
            if (foundvalues == nullptr) {
